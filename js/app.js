...
/**
 * STV count 진행 과정을 실험용 시각화로 렌더링한다.
 * @param {any} result STV 결과
 */
function renderStvVisualization(result) {
  stvVisualization.innerHTML = "";
  ...
  result.counts.forEach((count) => {
    ...
    const sourceCandidateName = resolveCandidateNameById(count.action.candidateId, result);
    const transferSummary = formatTransferSummaryByResult(count, result);
    const totalMovedVotes = count.action.transferredVotes + count.action.exhaustedVotes;
        ...
        <div class="d-flex flex-column flex-lg-row justify-content-between gap-2 mb-3">
          <div>
            <div class="d-flex flex-wrap gap-2 align-items-center mb-2">
              <span class="badge text-bg-dark">Count ${count.countNumber}</span>
              ${buildActionBadgeMarkup(count.action.type)}
            </div>
            <h4 class="h6 mb-1">${escapeHtml(count.action.description)}</h4>
            <p class="text-muted small mb-0">주체 후보: ${escapeHtml(sourceCandidateName)}</p>
          </div>
          <div class="d-flex flex-wrap gap-2">
            <span class="badge text-bg-primary">이동 ${totalMovedVotes}표</span>
            <span class="badge text-bg-secondary">소진 ${count.action.exhaustedVotes}표</span>
            <span class="badge text-bg-success">당선 ${count.newlyElected.length}명</span>
          </div>
        </div>
        <div class="mb-3">
          ${buildTransferVisualizationMarkup(count, result)}
        </div>
        <div class="small text-muted mb-3">이동 요약: ${escapeHtml(transferSummary)}</div>
    ...
    stvVisualization.appendChild(card);
  });
}

/**
 * count 작업 유형에 맞는 배지 마크업을 생성한다.
...
/**
 * count별 표 이동 흐름 마크업을 생성한다.
 * @param {any} count count 정보
 * @param {any} result 전체 결과
 * @returns {string} 흐름 마크업
 */
function buildTransferVisualizationMarkup(count, result) {
  if (!count.action.transfers || count.action.transfers.length === 0) {
    if (count.action.exhaustedVotes > 0) {
      return `
        <div class="border rounded p-3 bg-body-tertiary">
          <div class="fw-semibold mb-1">표 이동 없음</div>
          <div class="text-muted small">이 count에서는 다음 선호가 없어 ${count.action.exhaustedVotes}표가 소진되었습니다.</div>
        </div>
      `;
    }
    ...
    return `
      <div class="border rounded p-3 bg-body-tertiary">
        <div class="fw-semibold mb-1">표 이동 없음</div>
        <div class="text-muted small">이 count에서는 후보 상태만 갱신되었습니다.</div>
      </div>
    `;
  }
  ...
  const totalTransferred = Math.max(
    1,
    count.action.transfers.reduce((sum, transfer) => sum + transfer.votes, 0)
  );
  const sourceCandidateName = resolveCandidateNameById(count.action.candidateId, result);

  const bars = count.action.transfers
    .map((transfer) => {
      const recipientName = resolveCandidateNameById(transfer.candidateId, result);
      const width = Math.max(10, Math.round((transfer.votes / totalTransferred) * 100));
      return `
        <div class="mb-2">
          <div class="d-flex justify-content-between small mb-1">
            <span>${escapeHtml(sourceCandidateName)} -> ${escapeHtml(recipientName)}</span>
            <span>${transfer.votes}표</span>
          </div>
          <div class="progress" role="progressbar" aria-label="${escapeHtml(recipientName)} 표 이동" aria-valuenow="${width}" aria-valuemin="0" aria-valuemax="100">
            <div class="progress-bar" style="width: ${width}%">${transfer.votes}</div>
          </div>
        </div>
      `;
    })
  ...
  const exhaustedMarkup =
    count.action.exhaustedVotes > 0
      ? `<div class="small text-muted mt-2">다음 선호를 찾지 못한 ${count.action.exhaustedVotes}표는 소진되었습니다.</div>`
      : "";

  return `
    <div class="border rounded p-3 bg-body-tertiary">
      <div class="fw-semibold mb-2">${escapeHtml(sourceCandidateName)}에서 다른 후보로 이동한 표</div>
      ${bars}
      ${exhaustedMarkup}
    </div>
  `;
...
/**
 * count 종료 시점 후보별 현황 마크업을 생성한다.
 * @param {any} count count 정보
 * @returns {string} 표 본문 마크업
 */
function buildCountTallyMarkup(count) {
  return count.tallies
    .map((tally) => {
      return `
        <tr>
          <td>${escapeHtml(tally.candidate.name)} <span class="text-muted">(${escapeHtml(tally.candidate.party)})</span></td>
          <td>${tally.votes}</td>
          <td>${renderCandidateStatusText(tally)}</td>
        </tr>
      `;
 ...
 * @returns {string} 요약 문자열
 */
function formatTransferSummaryByResult(count, result) {
  if (!count.action.transfers || count.action.transfers.length === 0) {
    return count.action.exhaustedVotes > 0 ? `소진 ${count.action.exhaustedVotes}표` : "-";
  }

  const transferText = count.action.transfers
    .map((transfer) => `${resolveCandidateNameById(transfer.candidateId, result)} ${transfer.votes}표`)
    .join(", ");
